    ...
    if not st.session_state.range_selected:
        ...
        if st.button("Start Quiz"):
            if start_index <= end_index:
                ...
                st.session_state.start_number = start_index  # 保存起始題號
            else:
...
def record_wrong_answer(index, user_answer, question):
    ...
    actual_question_number = start_index + st.session_state.start_number  # 使用用戶選擇的起始題號
    ...
    st.session_state.wrong_answers.append({
        ...
        "original_number": actual_question_number,  # 使用實際題號
